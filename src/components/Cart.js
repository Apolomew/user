...
const Cart = (props) => { // props là các đối tượng chứa các thuộc tính được truyền vào cart
  const [cart, setCart] = useState([]);// khai báo một state cart với giá trị rỗng
  const history = useHistory(); //cho phép truy cập vào history object và sử dụng các hàm (push, replace, goback) để điều hướng trang

  useEffect(() => {
    onLoad();
  }, []); // Hook này được gọi một lần khi component được render lần đầu tiên với dependency array rỗng

  const onLoad = () => { //được sử dụng khi components được render hoặc thay đổi props
    if (props.user) {
